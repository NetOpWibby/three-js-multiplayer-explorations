...
class FirstPersonControls {
  ...
  updateControls() {
    ...
    const intersectionsDown = this.raycaster.intersectObjects(this.getCollidables());
    const onObject = intersectionsDown.length > 0 && intersectionsDown[0].distance < 0.25;
    // Here we talkin bout gravity...
    // this.velocity.y -= 9.8 * 8.0 * delta; // 100.0 = mass

    // For double-jumping!
    if (this.camera.position.y > 1.7 && this.gravity)
      this.gravity = 2.0; // less gravity like when we begin
    ...
    if (this.gravity)
      this.velocity.y -= 9.8 * this.gravity * delta; // Add gravity // 100.0 = mass
    else
      this.velocity.y = 0; // Otherwise don't and we stay suspended on the Y axis
  ...
  getCameraForwardDirAlongXZPlane() {
    ...
    const forwardAlongXZPlane = new THREE.Vector3(forwardDir.x, 0, forwardDir.z);
  ...
  detectCollisions() {
    ...
    const rightDir = forwardDir.clone().cross(new THREE.Vector3(0, 1, 0)).normalize();
    ...
    const pt = this.camera.position.clone();
  ...
  checkCollisions(pts, dir) {
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      const collisions = this.raycaster.intersectObjects(this.getCollidables());
